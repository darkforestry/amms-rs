...
use crate::errors::SwapSimulationError::ZeroAmountIn;
use crate::{
...
impl AutomatedMarketMaker for UniswapV3Pool {
    ...
    fn sync_from_log(&mut self, log: Log) -> Result<(), EventLogError> {
        ...
        return match event_signature {
            IUniswapV3Pool::Burn::SIGNATURE_HASH => Ok(self.sync_from_burn_log(log)?),
            IUniswapV3Pool::Mint::SIGNATURE_HASH => Ok(self.sync_from_mint_log(log)?),
            IUniswapV3Pool::Swap::SIGNATURE_HASH => Ok(self.sync_from_swap_log(log)?),
            _ => return Err(EventLogError::InvalidEventSignature),
        };
    }
    ...
    fn simulate_swap(
        ...
        match self.simulate_swap_(token_in, amount_in) {
            Ok(swap_state) => {
                let amount_out = (-swap_state.amount_calculated).into_raw();
                tracing::trace!(?amount_out);
                Ok((-swap_state.amount_calculated).into_raw())
            }
            Err(e) => match e {
                ZeroAmountIn => Ok(U256::ZERO),
                _ => Err(e),
            },
        }
    }
    ...
    fn simulate_swap_mut(
        ...
        match self.simulate_swap_(token_in, amount_in) {
            Ok(swap_state) => {
                self.liquidity = swap_state.liquidity;
                self.sqrt_price = swap_state.sqrt_price_x_96;
                self.tick = swap_state.tick;
...
impl UniswapV3Pool {
    ...
    /// Simulates a swap of a given amount of token_in.
    fn simulate_swap_(
        &self,
        token_in: Address,
        amount_in: U256,
    ) -> Result<CurrentState, SwapSimulationError> {
        if amount_in.is_zero() {
            return Err(ZeroAmountIn);
        }
            ...
            && current_state.sqrt_price_x_96 != sqrt_price_limit_x_96
        {
            ...
            // Target spot price
            let use_price_limit =
                zero_for_one && (step.sqrt_price_next_x96 < sqrt_price_limit_x_96);
            let swap_target_sqrt_ratio = if use_price_limit {
            ...
            if current_state.sqrt_price_x_96 == step.sqrt_price_next_x96 {
                if step.initialized {
                    if let Some(info) = self.ticks.get(&step.tick_next) {
                        let mut liquidity_net = info.liquidity_net;
