...
contract WethValueInPools {
    ...
    function getWethValueInPools(PoolInfo[] memory pools) public returns (PoolInfoReturn[] memory) {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            if (info.poolType == PoolType.Balancer) {
                uint256 wethValue = handleBalancerPool(IBPool(info.poolAddress));
                poolInfoReturns[i] = PoolInfoReturn(info.poolType, info.poolAddress, wethValue);
            } else if (info.poolType == PoolType.UniswapV2) {
                uint256 wethValue = handleUniswapV2Pool(IUniswapV2Pair(info.poolAddress));
                poolInfoReturns[i] = PoolInfoReturn(info.poolType, info.poolAddress, wethValue);
            } else if (info.poolType == PoolType.UniswapV3) {
                uint256 wethValue = handleUniswapV3Pool(IUniswapV3Pool(info.poolAddress));
                poolInfoReturns[i] = PoolInfoReturn(info.poolType, info.poolAddress, wethValue);
            }
        }
    ...
    function handleBalancerPool(IBPool pool) internal returns (uint256) {
        // Get pool tokens
        address[] memory tokens;
        try pool.getCurrentTokens() returns (address[] memory _tokens) {
            tokens = _tokens;
        } catch {
            return 0;
        }
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            if (tokens[i] == WETH) {
                try pool.getBalance(tokens[i]) returns (uint256 _balance) {
        ...
        uint256 balance;
        try pool.getBalance(baseToken) returns (uint256 _balance) {
            balance = _balance;
        } catch {
            return 0;
        }
    ...
    function handleUniswapV2Pool(IUniswapV2Pair pool) internal returns (uint256) {
        address token0;
    ...
    function handleUniswapV3Pool(IUniswapV3Pool pool) internal returns (uint256) {
        address token0;
        try pool.token0() returns (address _token0) {
            token0 = _token0;
        } catch {
...
contract WethValueInPoolsBatchRequest is WethValueInPools {
    constructor(address _uniswapV2Factory, address _uniswapV3Factory, address _weth, PoolInfo[] memory pools)
        WethValueInPools(_uniswapV2Factory, _uniswapV3Factory, _weth)
    {
        PoolInfoReturn[] memory poolInfoReturn = getWethValueInPools(pools);
